...
submitForm() {
 ...
 checkDate(event:Event){
  let start_date = localStorage.getItem('start_date');
   if((event.target as HTMLInputElement).getAttribute('name')=='start_date'){
   localStorage.setItem('start_date',(event.target as HTMLInputElement).value)
   }else if(!start_date || start_date > (event.target as HTMLInputElement).value) {
